...
## Phase 2 — 고전 제어 (학습일 2~3일, 세션 4개) — 이론 완료, 프로젝트 보류 중
...
- [x] 보드 선도 읽기/그리기, 이득 여유·위상 여유 (Session 3, phase2-bode-problems.md)
- [x] **root locus** — K가 변할 때 pole이 그리는 궤적. Session 2에서 손으로 이미 한 것(W2의 −(2+K), W5의 수직선)의 이름과 작도법. Bode와 짝: root locus는 pole의 이동, Bode는 주파수별 이득·위상 (Session 3에 포함)
- [~] 📌 **프로젝트** (Session 4, phase2-project.md): **모터 위치 제어기 설계** — Part A 시뮬레이션(M1~M4 완료, **M5 일부 진행 후 2026-07-28 보류** — 재개 지점은 phase2-project.md의 "M5 중단 기록" 참조) → Part B Dynamixel 실물. 시뮬을 실물의 예행연습으로 삼아 같은 plant 구조로 설계 후 검증하기로 결정(크루즈 컨트롤 대신). 실물로 하면 system identification(PWM 모드 step 응답으로 G·H 동정)이 앞에 붙고, loop rate를 낮춰가며 발진 지점을 찾아 phase margin을 실측할 수 있음. Phase 3의 my_iir을 filtered derivative 구현에 재사용
...
## ✅ Phase 4 — 선형대수 재무장 (완료: 2026-07-28, 세션 4개 / 하루)

상태공간 진입 직전 배치. 최대 빈칸이지만 목표가 명확해 빠르게 간다.
**실측**: 예상 3~4 학습일이었으나 하루에 4세션으로 완주 — "필요해지기 직전에 배치" 원칙이
동기를 만들어준 덕. 구조적으로 **Session 1(동치 사슬)이 이후 전 세션에서 계속 회수**됐다
(특성방정식 = 차원 붕괴 조건, 최소제곱 = 열공간 밖의 b, SVD = 0이 아닌 σ의 개수).

- [x] **동치 사슬 하나로 꿰기**: rank ↔ det≠0 ↔ 열 선형독립 ↔ 역행렬 존재 (Session 1, phase4-rank-problems.md)
- [x] 대각화 A = PDP⁻¹ (아는 고유값 직관에서 한 걸음), e^(At)이 대각화로 쉬워지는 이유 + 특성방정식 det(A−λI)=0, eigenspace, **안정성 = eigenvalue** (Session 2, phase4-eigen-problems.md)
- [x] 최소제곱법 — 이미 아는 정사영 (uᵀb)u의 일반화로 접근 + 정규방정식 AᵀAx̂=Aᵀb, full column rank 조건 (Session 3, phase4-leastsquares-problems.md)
- [x] 양의 정부호 행렬, SVD는 개념만 (Session 4, phase4-posdef-svd-problems.md)
- [x] 그래디언트 = 최급상승 방향 (5분 보충) — 내적 `∇f·û = |∇f|cosθ`에서 유도 (Session 4)

**자료**: 3Blue1Brown *Essence of Linear Algebra* 전편 · MIT 18.06 (Strang) 선별 강의

## Phase 5 — 상태공간 제어 (학습일 2~3일, 세션 3~4개) ← 다음 차례
...
| 2026-07-25 | Phase 2 — Session 3 착수 (Bode, 안정 여유) — **D1·D2 완료, D3(a)(b)까지** | 같은 날 Session 2에 이어 진행. **개념 질문이 세션 내용을 끌고 간 회차** — "특성방정식이 뭔지부터 보자", **"왜 하필 jω를 대입해?"**(→ 허수축=안정 경계 + **물리적으로 측정 가능한 유일한 축**, Bode가 모델 없이도 그려지는 이유, Phase 1의 X(jω) 회수), "ω는 여러 개 넣어보는 거야?"(전 대역 sweep, 같은 ω에서 두 조건이 겹칠 때만 발진). D1 전 문항 자력: 허수축 pole=영구진동, **`E = R + L·E`로 negative→positive feedback 자력 도달**, `0.5ⁿ→0`을 Phase 3의 단위원 pole과 같은 구조로 연결. 파생 질문 둘이 특히 좋았음 — ✍️ **"둘 중 하나만 피해도 되는 거 아니야?"**(안정성만 놓고 보면 **정확한 지적**; 둘 다 보는 건 GM=이득 변동, PM=위상/지연 변동이라는 **서로 다른 불확실성 보험**이기 때문 + PM만 `ζ≈PM/100`으로 품질까지 결정), ✍️ **"둘을 같이 보는 유클리디안 거리로 재는 게 좋아?"** → **vector margin `min|1+L| = 1/max|S|`에 자력 도달**, Bode(두 장)로는 안 보이고 Nyquist(복소평면)라야 보인다는 점과 **H∞가 정확히 그걸 최소화하는 분야**임까지 연결(B5의 S 재등장). D2: 부품별 점근선 합산 자력(저주파 −20, 고주파 −40 dB/dec), ⚠️ corner에서 `10^0.3` 대신 `e^0.3`(1.34) 1회 → ω=2 교정, **corner = |pole|** 규칙 및 **고주파 기울기÷(−20) = pole 개수**(← "내 모터가 몇 차인지" 질문의 실측 답) 획득. GM은 첫 답 "0에 가까워져" 오답 → **GM=∞** 교정, **pole 하나당 위상 −90°라 2개짜리는 원리적으로 발진 불가**로 W5의 수직선과 동일 사실임을 확인. D3: `ω⁴+4ω²−64=0` 자력 → ω_gc=2.5, PM=38.7° 자력(위상 합산법은 해설 필요). 세션 외 논의: **Dynamixel 적용 방안**(PWM 모드로 loop을 열어 G·H 동정 → 내장 PID 게인으로 P/PI/PID 재현 → 외부 PID 직접 구현, loop rate를 낮춰가며 발진 지점으로 phase margin 실측), **G와 H는 분리 측정 불가**(곱만 관측됨 → G'=GH로 두고 unity feedback 취급, 대가는 센서 오차를 원리적으로 감지 못함 = calibration이 필요한 이유), 엔코더는 H≈순수 gain이라 운이 좋은 경우, 감속기의 backlash·비틀림 공진. **Session 3 후반 (같은 날 이어서, D3(c)~D6 + Python 검증 완주)**: D3(c) `ζ≈PM/100 = 0.387` vs W4 손계산 0.354(9% 차) — **근을 한 번도 안 풀고 응답 품질을 알아냄**. **D4가 오늘 가장 값나가는 문제** — `T_max = 270ms` 자력, 100Hz에서 55탭 자력, PM 45° 유지 조건을 계산하니 예산이 **−6.3°**로 나오자 ✍️ "이미 PM이 38.7인데 45를 어떻게 유지해"로 함정을 자력 간파. 여기서 loop shaping 도입(**위상 곡선은 K와 무관한 고정 지형, K는 crossover를 어디에 내려놓을지만 정함** — pole placement와 대비되는 두 번째 설계 방식). PM 60° 확보 설계를 역산해 ω_gc=1.154, K=2.66, 지연 예산 0.22초, **45탭** 전부 자력 → **게인을 3배 포기해서 45탭 필터를 샀다**는 거래를 숫자로 완성. ⚠️ 결론에서 "GM을 깎아"를 PM으로 교정(지연은 `|e^(−jωT)|=1`이라 크기를 안 건드림). 탭당 단가 0.33°가 ω_gc에 비례 → **빠른 시스템일수록 필터가 비싸다**, Phase 3의 IIR 우위를 "같은 지연 예산으로 더 많은 노이즈를 산다"로 재해석. D5: `s³+12s²+20s+K`, `s=jω` 분리로 **ω=√20, K=240** 자력. ⚠️ "예산에 제한이 생겼어"는 방향 반대 → 위상 예산이 **늘어나서**(−270°까지 가능) −180°를 유한 ω에서 통과하게 된 것으로 교정. "K를 키우면?"의 답이 **W2(빨라짐) → W5(진동만) → D5(발산)** 로 3단 갱신되며 B3의 "K 상한이 있다"가 240이라는 숫자로 확정. D6: 점근선 60/180/300° 자력, ✍️ **"pole이 늘면 각도가 작아지고 더 빨리 우반평면으로 간다"** 자력 — `180°/(n−m)` 공식으로 **n−m≤2 안전 / ≥3 위험**이 Bode의 위상 예산과 같은 문턱임을 확인. Python 검증 전 항목 일치(ω_gc 2.4992, PM 38.67°, T=270ms에서 PM 0.01°, K=240에서 근이 정확히 ±j4.472), **네 지연 조건 모두 ω_gc가 2.499로 고정**된 것이 "지연은 위상만 먹는다"의 시각적 증거. **Phase 2 이론 3세션 완료.** |
| 2026-07-25 | Phase 2 — Session 4 프로젝트 착수 (모터 위치 제어기 설계) — **M1~M4 완료** | 프로젝트 대상을 크루즈 컨트롤 대신 **모터 위치 제어**로 변경 — 시뮬(Part A)이 Dynamixel 실물(Part B)의 예행연습이 되도록. 설계 모델 `G=100/(s(s+10))`, 실제 plant는 `s=−200`에 전기 pole이 숨어 있고 M6에서 드러냄. **M1**: pole·적분기·Bode 자력, 💬 "open-loop step은 어떻게 알아?"에 pole 읽기/FVT 발산 확인/물리 세 방법 정리 → **적분기 있는 plant는 open-loop 위치 제어가 원리적으로 불가능**(B5의 open-loop compensation은 `G(0)` 유한이라 가능했던 것). **M2가 개념적 핵심** — `Y/D = G/(1+CG)` 자력 유도 후 `(Y/D)(0) = 1/K_p ≠ 0` 도출: **plant에 적분기가 있어 setpoint 오차는 0인데 외란 오차는 남는다**(정상상태에 필요한 `u`가 0 vs −d로 다르기 때문, B4의 "오차 없으면 미는 힘도 없다"가 외란 쪽에서 재발). Dynamixel 수평 팔 처짐 = d/K_p. 손잡이 3개 vs 계수 3개 논법으로 PID 세 항 전부 필요함을 확인. **M3에서 설계가 한 번 실패** — pole은 소수점까지 정확히 배치됐는데 overshoot 14.9%로 FAIL, 원인은 **PID가 스스로 만든 zero**(P5에서 튜터가 걸렸던 함정에 이번엔 설계가 걸림). derivative-on-measurement로 고치려 했으나 **오히려 33.6%로 악화** — ✍️ 자력 진단: zero **개수**가 아니라 **위치**(원점에 더 가까워짐, `|z|/ζωₙ`가 1.55→1.09). 💬 튜터 정정: D-on-measurement의 확실한 효과는 `u`의 kick 제거이지 overshoot 감소가 아님. ✍️ **"근데 zero가 나쁜 거야?"** 질문에서 **`L`의 zero(위상 벌어옴, 좋음)와 `T`의 zero(overshoot, 나쁨)가 같은 zero**라는 구조 도달 → setpoint weighting이 그 둘을 분리하는 장치임을 이해하고 **I-PD로 PASS**(OS 8.8%, 정착 0.474s, zero 없음, 외란 응답은 세 구조 동일). ✍️ 파생 질문 둘: "실제로는 Y를 못 얻으니 D를 못 걸잖아"(→ `y`가 오히려 원본이고 `r` 상수 구간에선 `ė=−ẏ`), **"Present Velocity가 내부적으로 어떻게 계산되는지 모르는 거 아니야?"**(→ 정확한 의심, 내부 필터의 `τ_f`·지연 미상 = D5의 "모델에 없는 필터" 상황 → Part B 실험으로 추가). **M4**: PM 79.4°, GM=∞, VM=1.0으로 사양 초과 달성. 적분기 2개라 위상이 −180°에서 출발하는 **type-2** 구조를 확인하고, PM 전부가 zero에서 왔음을 위상 장부로 검산(−180 −77.5 +156.9 = −100.6). **zero가 없으면 어떤 게인으로도 불안정**(I만 쓰면 `s` 계수가 0이라 Routh 불만족) → **`K_p`·`K_d`는 성능 손잡이가 아니라 안정성을 사는 대가**, "적분기 하나당 위상 −90°의 빚"이라는 정리 획득. 다음: **M5(노이즈 → filtered derivative, 예산 34.4°)부터, 여기서부터 사용자 구현** |
| 2026-07-28 | Phase 2 — M5 일부 진행 후 **프로젝트 보류**, Phase 4로 전환 | 파일 표기를 LaTeX으로 전환(phase2-project.md, Obsidian MathJax). M5: ✍️ "노이즈를 가장 키우는 항은 D" 자력 정답, `K_d·ω` 표를 채워 **D가 P를 추월하는 지점 ω≈17.7이 ω_gc=45보다 아래**임을 확인(소수점 밀림 1회는 "10배면 몇 배?" 미니 테스트로 자가 교정). 💬 정상상태 `u=0`(속도 0 → 점성 마찰도 0) — **이 때문에 출제된 "정상상태 u의 몇 배" 문항이 0으로 나누기라 성립 안 함을 튜터가 인정·수정**. 💬 백색 노이즈 = 모든 주파수 균등. **남은 것: "표가 어디서 멈추나" → σ_u가 유한하지 않음 → 순수 미분 구현 불가** (재개 지점, phase2-project.md의 "M5 중단 기록"에 상세). ⚠️ 이날 정체는 내용보다 **진행 방식** 문제 — 코드 전에 예측 질문 3개를 앞세웠고 그중 하나가 성립 안 하는 문항이었음. 사용자가 "너무 어렵다 → 넘어갈까" 제기 → Phase 2 이론은 이미 완료됐고 Phase 4~6이 프로젝트에 의존하지 않으므로 **보류 후 Phase 4(선형대수)로 전환**. 프로젝트는 Dynamixel 실물 만질 때 복귀 |
| 2026-07-28 | **Phase 4 — Session 1 완료 (동치 사슬)** 🎓 | 같은 날 전환 후 바로 진행, **전 문항 자력**. W0에서 "변환 후 기저벡터가 움직인다" 자력 → **행렬의 열 = 기저벡터가 도착한 자리** 획득(이 세션의 나머지가 전부 여기서 파생). L1에서 `(2,4)=2·(1,2)` 자력 발견 → 평면이 직선으로 뭉개짐. L2 "못 알아내지" 자력 — 서로 다른 입력이 같은 출력이라 되돌릴 수 없음 = 역행렬 없음의 뜻. det A=6(단위정사각형→넓이 6)·det B=0 검산 자력. **L3이 이 세션의 핵심** — 3×3 두 개가 둘 다 det=0인데 "각각 2차원, 1차원" 자력 판별 → **det은 예/아니오, rank는 몇 차원인지**. Phase 5의 가제어성이 det이 아닌 rank로 판정되는 이유 두 가지(입력 다개수면 비정사각 → det 미정의 / 못 하는 방향의 개수가 필요) 연결. 진행 방식을 **작은 계산 하나 → 즉시 확인**으로 바꾼 것이 M5 정체와의 유일한 차이 |
| 2026-07-28 | **Phase 4 — Session 2 완료 (대각화·안정성)** 🎓 | 같은 날 이어서. A=[[2,1],[1,2]]로 E1~E5 계산 전부 자력: eigenvector "방향 그대로 크기만" 발견 → (2,0)을 eigen 조합으로 쪼개 **두 경로(eigen vs 직접) 일치를 손으로 확인** → A¹⁰에서 3¹⁰ vs 1¹⁰ → **dominant 방향이 Phase 1·2의 dominant pole과 같은 구조**임을 확인, e^(At)=P·e^(Dt)·P⁻¹까지. **세션의 질을 올린 건 사용자의 지적 두 개** — ✍️ **"P랑 P⁻¹은 어떻게 구하는지 안 다뤘잖아"**(튜터가 eigenvector를 건네주기만 하고 넘어간 빈칸을 포착) → 특성방정식을 채우면서 **det(A−λI)=0이 Session 1의 동치 사슬을 도구로 쓰는 구조**임이 드러남(0 아닌 v가 0으로 가려면 차원이 무너져야 한다), ✍️ **"여러 개가 존재하잖아"**(해집합이 직선 전체 → **eigenspace** 자력 도달, A·v=λv가 크기에 무관하므로). P⁻¹ 공식·detP=−2·PDP⁻¹=A 검산 자력. 마지막에 **안정성 = eigenvalue**로 Phase 5 입구 도달(연속 Re(λ)<0 ↔ Phase 1 좌반평면, 이산 \|λ\|<1 ↔ Phase 3 단위원). ⚠️ **연속/이산 혼동 재발** — λ=1을 "유지"로 답함(이산은 맞지만 연속은 e^t라 폭발). Phase 3 I4의 s=−1 오독과 같은 패턴이라 **감시 대상 승격**, 미니 테스트(t=0,1,10)로 자가 교정. 다음: Session 3(최소제곱 = 정사영 일반화, Phase 6 칼만 게인의 뿌리) |
| 2026-07-28 | **Phase 4 — Session 3 완료 (최소제곱)** 🎓 | 같은 날 세 번째 세션, **전 문항 자력**. L1에서 3×2 A에 대해 "택도 없는데? (x₁,x₂,0)" — 열공간 밖의 b를 즉시 파악(Session 1의 **열 = 도착지**가 도구로 재사용). L2에서 최근접점 (1,2,0)·오차 (0,0,3)·**수직 관계**까지 자력 → **"가장 가깝다 = 오차가 수직이다"**가 최소제곱의 전부임을 확인(피타고라스). L3: 각 열과의 내적=0 자력 → Aᵀe=0으로 압축(해설) → **정규방정식 AᵀAx̂=Aᵀb** 도달, 해가 없는 식을 해가 있는 식으로 바꾼 구조. L4 검산에서 AᵀA=I, x̂=(1,2)로 눈으로 찍은 답과 일치. **정사영 (uᵀb)u가 "열이 하나일 때의 최소제곱"**임을 대입으로 확인 — 로드맵의 "아는 것에서 출발" 설계가 그대로 작동. 마지막에 ✍️ **"rank가 무너진 상태?"** 자력 → full column rank 조건, 단 **투영점은 여전히 유일하고 계수 x̂만 안 정해진다**는 정밀화, pseudo-inverse·SVD 예고. 📌 Phase 6 연결: 가중 최소제곱(wᵢ=1/σᵢ²)의 가중치가 곧 **칼만 게인**, 칼만은 그걸 재귀적으로 하는 것. **Phase 4는 Session 1이 계속 회수되는 구조**(특성방정식도, 최소제곱도). 남은 것: 양의 정부호·SVD(개념만)·그래디언트 |
| 2026-07-28 | **Phase 4 전체 완료** 🎓 — Session 4 (양의 정부호·SVD·그래디언트) | 하루에 4세션으로 Phase 4 완주(예상 3~4 학습일). ① 양의 정부호: `xᵀAx`를 eigenvector로 계산해 **`xᵀAx = λ|x|²` 자력 발견** → 일반 x를 쪼개면 제곱들만 남아 **부호를 λ가 정한다**로 일반화, LQR 비용이 음수면 최적화가 망가진다는 동기와 공분산의 준정부호성(Phase 6)까지. ② SVD: ✍️ **"σᵢ는 어디서 나온 거야?"가 이 세션 최고의 질문** — `σ = √(AᵀA의 eigenvalue)`로 내려놓으니 Session 1(rank)·2(eigenvalue)·3(AᵀA)이 한 자리에 모였고, **제곱근이 실수인 근거가 바로 앞 ①**(`xᵀAᵀAx = |Ax|² ≥ 0` 한 줄 증명). "0 아닌 σ의 개수 = rank"는 해설 필요. 실무 관점 획득: **det/rank는 이산적(무너졌나), σ는 연속적(얼마나 무너지기 직전인가)** → 조건수, Phase 6 공분산 수치 문제. ③ 그래디언트: `∇f·û = |∇f|cosθ`에서 θ=0 자력 — **"가장 가파른 방향"이 정의가 아니라 내적에서 유도되는 결과**. **Phase 4의 구조적 특징: Session 1(동치 사슬)이 이후 전 세션에서 계속 회수됨.** 다음: **Phase 5 상태공간** (안정성 = A의 고유값이 Session 2에서 이미 준비됨) |
